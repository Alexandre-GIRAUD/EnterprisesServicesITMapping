...
/**
 * Reads the distinct values currently stored for one dynamic {@code Application} property.
 *
 * <p>The property key becomes a Cypher identifier, so callers MUST pass a key already validated
 * against the Data Model.
 */
@Repository
public class ApplicationNodeAttributeFacetQuery {

  private final Neo4jClient neo4jClient;

  public ApplicationNodeAttributeFacetQuery(Neo4jClient neo4jClient) {
  ...
  public List<String> distinctValues(String validatedKey) {
    String cypher =
        ...
        WITH DISTINCT toString(a.`%s`) AS value
        WHERE trim(value) <> ''
